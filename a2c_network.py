...
def conv_layer(inputs, h_size, filter_size, strides):
    return tf.keras.layers.Conv2D(h_size, (filter_size, filter_size), strides=(strides, strides),
                                  padding='same', kernel_initializer='glorot_uniform')(inputs)

def dense_layer(inputs, units, activation=None, std=0.1):
    return tf.keras.layers.Dense(units, activation=activation, kernel_initializer='random_normal',
                                 bias_initializer=tf.constant_initializer(1.))(inputs)
...
class ActorCritic(object):
    ...
    def pick_action(self, state):
        result = self.sess.run(self.action_probs,
